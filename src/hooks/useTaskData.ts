/**
 * Main data hook — useQuery/useMutations for collection data.
 *
 * Data lives in the app scope with PascalCase column names.
 ...
 * remain stable across real-time updates.
 */
...
export function useTaskData(currentUser: WidgetUser | null, teamId: string | null) {
  ...
  // ── Map PascalCase records → camelCase app objects ──
  ...
  const reorderTask = useCallback(
    (draggedId: string, targetId: string) => {
      const allTaskRecords = taskRecordsRef.current || [];
      const sorted = [...allTaskRecords]
